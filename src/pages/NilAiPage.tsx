...
const WELCOME_MESSAGE: ChatMessage = {
  ...
  content: [
    ...
    "",
    "I can query parcels across **Khutal** (Murbad, Thane) village sectors or compile a cadastral analysis report with map layouts and attribute tables.",
    "",
...
export default function NilAiPage() {
  ...
  const parcelFeatures = useMemo(() => getParcelFeatures(DEFAULT_REGION_KEY), []);
      ...
      window.setTimeout(() => {
        ...
        if (result.parcelIds.length) {
          mapRef.current?.highlightParcels(result.parcelIds, {
            colorByVariance: result.highlightMode === "variance",
          });
        } else {
